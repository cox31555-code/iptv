      ...
      <section className={`relative w-full flex items-center justify-center overflow-hidden transition-all duration-700 ${isSearching ? 'min-h-[300px] md:min-h-[350px]' : 'min-h-[350px] md:min-h-[420px]'}`}>
        ...
        <div className={`max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-30 w-full text-center transition-all duration-500 ${isSearching ? 'pt-10 md:pt-14 pb-6' : 'pt-8 md:pt-10 pb-10 md:pb-12'}`}>
          ...
          <div className="max-w-2xl mx-auto px-4 transition-all duration-500 mt-8">
            ...
            <div className="flex items-center justify-center gap-6 md:gap-8 mt-6 md:mt-8 animate-in fade-in slide-in-from-bottom-2 duration-1000 delay-500">
              <a href="https://twitter.com/ajsportstv" target="_blank" rel="noopener noreferrer" className="group flex items-center gap-2 text-zinc-500 hover:text-white transition-all">
                <svg role="img" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" className="w-5 h-5 fill-current group-hover:text-white transition-all">
                  <title>X</title>
                  <path d="M14.234 10.162 22.977 0h-2.072l-7.591 8.824L7.251 0H.258l9.168 13.343L.258 24H2.33l8.016-9.318L16.749 24h6.993zm-2.837 3.299-.929-1.329L3.076 1.56h3.182l5.965 8.532.929 1.329 7.754 11.09h-3.182z"/>
                </svg>
                <span className="text-[9px] font-black uppercase tracking-[0.2em]">Follow on X</span>
              </a>
              <a href="http://t.me/ajsports" target="_blank" rel="noopener noreferrer" className="group flex items-center gap-2 text-zinc-500 hover:text-white transition-all">
                <svg role="img" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" className="w-5 h-5 fill-current group-hover:text-[#26A5E4] group-hover:drop-shadow-[0_0_8px_rgba(38,165,228,0.5)] transition-all">
                ...
                <span className="text-[9px] font-black uppercase tracking-[0.2em]">Telegram</span>
              </a>
              <a href="https://discord.gg/M9QuKhp8Hb" target="_blank" rel="noopener noreferrer" className="group flex items-center gap-2 text-zinc-500 hover:text-white transition-all">
                <svg role="img" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" className="w-5 h-5 fill-current group-hover:text-[#5865F2] group-hover:drop-shadow-[0_0_8px_rgba(88,101,242,0.5)] transition-all">
                  <title>Discord</title>
                ...
                <span className="text-[9px] font-black uppercase tracking-[0.2em]">Discord</span>
              </a>
            </div>
          </div>
        </div>
              ...
              if (sectionName === EventCategory.FOOTBALL && liveEvents.length > 0) {
                blocks.push(
                  <section key="live-now" className="space-y-6 md:space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
                    ...
                    {/* Horizontal scroll for Live events */}
                    <div className="flex overflow-x-auto snap-x snap-mandatory gap-4 md:gap-8 pb-4 no-scrollbar -mx-2 px-2">
                      {liveEvents.map(event => (
                        <div key={event.id} className="w-44 sm:w-64 md:w-72 lg:w-80 shrink-0 snap-start">
                    ...
                    {isSpecial ? (
                      <div className="relative">
                        <div 
                          ...
                          className="flex overflow-x-auto snap-x snap-mandatory gap-4 md:gap-6 no-scrollbar pb-4 -mx-2 px-2"
                        >
                          {items.map(event => (
                            <div key={event.id} className="min-w-[85vw] md:min-w-[400px] lg:min-w-[450px] snap-center">
                              <EventCard event={event} />
                            </div>
                          ))}
                        </div>
